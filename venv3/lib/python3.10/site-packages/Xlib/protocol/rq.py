...
class Field:
    ...
    def parse_binary_value(self, data, display, length, format):
        ...
        raise RuntimeError(
            f'Neither structcode or parse_binary_value provided for {self}'
        )
...
class Resource(Card32):
    ...
    def parse_value(self, value, display):
        ...
        if c := display.get_resource_class(self.class_name):
...
class Set(ValueField):
    ...
    def check_value(self, val):
        if val not in self.values:
            raise ValueError(f'field {self.name}: argument {val} not in {self.values}')
...
class String8(ValueField):
    ...
    def parse_binary_value(self, data, display, length, format):
        ...
        slen = length + ((4 - length % 4) % 4) if self.pad else length
        s = data[:length]
        try:
            s = s.decode('UTF-8')
        except UnicodeDecodeError:
...
class String16(ValueField):
    ...
    def pack_value(self, val):
        ...
        pad = b'\0\0' * (slen % 2) if self.pad else b''
    ...
    def parse_binary_value(self, data, display, length, format):
        ...
        slen = length + (length % 2) if self.pad else length
...
class List(ValueField):
    ...
    def pack_value(self, val):
        ...
        if self.type.structcode and len(self.type.structcode) == 1:
            ...
            data = [self.type.pack_value(v) for v in val]
            data = b''.join(data)
...
class FixedList(List):
    ...
    def pack_value(self, val):
        if len(val) != self.size:
            raise BadDataError(f'length mismatch for FixedList {self.name}')
...
class Object(ValueField):
    ...
    def pack_value(self, val):
        ...
        if self.type.structcode and len(self.type.structcode) == 1:
            return struct.pack(f'={self.type.structcode}', val), None, None
        else:
    ...
    def check_value(self, val):
        ...
        if type(val) is dict:
            ...
            raise TypeError(
                f'Object value must be tuple, dictionary or DictWrapper: {val}'
            )


        return [data[f.name] for f in self.type.fields if f.name]
...
class PropertyData(ValueField):
    ...
    def parse_binary_value(self, data, display, length, format):
        length = len(data) // (format // 8) if length is None else int(length)
        if format == 0:
            ret = None
            return ret, data
...
class FixedPropertyData(PropertyData):
    ...
    def pack_value(self, value):
        ...
        if len(data) != self.size:
            raise BadDataError(f'Wrong data length for FixedPropertyData: {value}')
    ...
    def pack_value(self, arg, keys):
        ...
        for field, flag in self.fields:
            if field.name in arg:
                ...
                d = struct.pack(f'={field.structcode}', val)
        ...
        for field, flag in self.fields:
            if mask & flag:
                if field.structcode:
                    vals = struct.unpack(
                        f'={field.structcode}',
                        data[: struct.calcsize(f'={field.structcode}')],
                    )
...
class KeyboardMapping(ValueField):
    ...
    def parse_binary_value(self, data, display, length, format):
        dlen = len(data) if length is None else 4 * length * format
        a = array(array_unsigned_codes[4], data[:dlen])

        ret = [a[i : i + format] for i in range(0, len(a), format)]
...
class ModifierMapping(ValueField):
    ...
    def parse_binary_value(self, data, display, length, format):
        ...
        ret = [a[i * format : (i + 1) * format] for i in range(8)]
...
class EventField(ValueField):
    ...
    def pack_value(self, value):
        if not isinstance(value, Event):
            raise BadDataError(f'{value} is not an Event for field {self.name}')
...
class StrClass:
    ...
    def pack_value(self, val):
        if type(val) is not bytes:
            val = val.encode('UTF-8')
        val = bytes([len(val)]) + val if _PY3 else chr(len(val)) + val
        return val
...
class Struct:
    ...
    def to_binary(self, *varargs, **keys):
        ...
        for f in self.var_fields:
            ...
            total_length = total_length + f' + len(_{f.name})'
            joins.append(f'_{f.name}')
        ...
        for f in self.static_fields:
            if isinstance(f, LengthField):
                ...
                pack_args.append(f'_{f.name}_format')
                ...
                if f.structvalues == 1:
                    ...
                    a = ['_%s_%d' % (f.name, j) for j in range(f.structvalues)]
                    if f.check_value is not None:
                ...
                if f.name:
                    if f.default is None:
                        ...
                        defargs.append(f'{f.name} = {repr(f.default)}')

            i = i + 1

        # Construct call to struct.pack
        pack = f"struct.pack({', '.join(pack_args)})"
    ...
    def pack_value(self, value):
        ...
        if type(value) is tuple:
            ...
            raise BadDataError(f'{value} is not a tuple or a list')
    ...
    def parse_value(self, val, display, rawdict = 0):
        ...
        for f in self.static_fields:
            ...
            if (
                f.name
                and not isinstance(f, LengthField)
                and not isinstance(f, FormatField)
            ):
...
class ReplyRequest(GetAttrData):
    ...
    def __repr__(self):
        return f'<{self.__class__} serial = {self._serial}, data = {self._data}, error = {self._error}>'
    ...
    def __repr__(self):
        ...
        for kw, val in self._data.items():
            ...
            if kw == 'type' and self._data['send_event']:
                val = val | 0x80
            kwlist.append(f'{kw} = {repr(val)}')

        kws = ', '.join(kwlist)
        return f'{self.__class__}({kws})'
