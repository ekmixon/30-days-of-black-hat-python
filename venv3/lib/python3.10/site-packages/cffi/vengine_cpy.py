...
class VCPythonEngine(object):
    ...
    def find_module(self, module_name, path, so_suffixes):
        ...
        return None if descr[0] not in so_suffixes else filename
    ...
    def _convert_funcarg_to_c(self, tp, fromvar, tovar, errcode):
        ...
        if isinstance(tp, model.PrimitiveType):
            if tp.is_integer_type() and tp.name != '_Bool':
                ...
                converter = f"({tp.get_c_name('')})_cffi_to_c_{tp.name.replace(' ', '_')}"
            errvalue = '-1'
    ...
    def _convert_expr_from_c(self, tp, var, context):
        if isinstance(tp, model.PrimitiveType):
            if tp.is_integer_type() and tp.name != '_Bool':
                ...
                return f"_cffi_from_c_{tp.name.replace(' ', '_')}({var})"
            else:
    ...
    def _generate_cpy_function_decl(self, tp, name):
        ...
        for i, type in enumerate(tp.args):
            prnt(f"  {type.get_c_name(' x%d' % i, context)};")
        #
        localvars = set()
        freelines = set()
        for type in tp.args:
        ...
        if not isinstance(tp.result, model.VoidType):
            ...
            prnt(f"  {tp.result.get_c_name(' result', context)};")
    ...
    def _generate_cpy_const(self, is_int, name, tp=None, category='const',
        ...
        prnt('  int res;')
        if not is_int:
            prnt(f"  {(vartp or tp).get_c_name(' i', name)};")
        else:
        ...
        #
        if not is_int:
            realexpr = f'&{name}' if category == 'var' else name
            prnt(f'  i = ({realexpr});')
        ...
        self._chained_list_constants[delayed] = f'{funcname}(lib)'
        prnt('}')
        prnt()
    ...
    def _generate_cpy_enum_decl(self, tp, name, prefix='enum'):
        ...
        for enumerator, enumvalue in zip(tp.enumerators, tp.enumvalues):
            self._check_int_constant_value(enumerator, enumvalue, f"enum {name}: ")
        prnt(f'  return {self._chained_list_constants[True]};')
        self._chained_list_constants[True] = f'{funcname}(lib)'
        prnt('}')
        prnt()
