...
class Verifier(object):
    ...
    def __init__(self, ffi, preamble, tmpdir=None, modulename=None,
        ...
        if modulename:
            ...
            k1 = hex(binascii.crc32(key[::2]) & 0xffffffff)
...
def _caller_dir_pycache():
    if _TMPDIR:
        return _TMPDIR
    if result := os.environ.get('CFFI_TMPDIR'):
        return result
...
def cleanup_tmpdir(tmpdir=None, keep_so=False):
    ...
    try:
        ...
        return
    suffix = '.c' if keep_so else _get_so_suffixes()[0].lower()
    for fn in filelist:
