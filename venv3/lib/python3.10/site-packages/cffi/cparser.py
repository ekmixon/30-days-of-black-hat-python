...
def _workaround_for_old_pycparser(csource):
    ...
    while True:
        ...
        if not match:
            break
        parts.extend((csource[:match.start()], '(', match.group()))
        endpos = match.end()
        closing = ')'
...
class Parser(object):
    ...
    def _parse(self, csource):
        ...
        csourcelines.extend(f'typedef int {typename};' for typename in typenames)
        csourcelines.extend(
            (
                'typedef int __dotdotdotint__, __dotdotdotfloat__,'
                ' __dotdotdot__;',
                '# 1 "%s"' % (CDEF_SOURCE_STRING,),
                csource,
            )
        )
    ...
    def _convert_pycparser_error(self, e, csource):
        ...
        line = None
        msg = str(e)
        if match := re.match(r"%s:(\d+):" % (CDEF_SOURCE_STRING,), msg):
            linenum = int(match[1], 10)
        ...
        try:
            ...
            for decl in iterator:
                ...
                if isinstance(decl, pycparser.c_ast.Decl):
                    ...
                    self._declare(f'typedef {decl.name}', realtype, quals=quals)
            ...
            if msg := self._convert_pycparser_error(e, csource):
                e.args = (e.args[0] + "\n    *** Err: %s" % msg,)
            raise
    ...
    def _add_integer_constant(self, name, int_str):
        ...
        if (int_str.startswith("0") and int_str != '0'
            ...
            int_str = f"0o{int_str[1:]}"
        pyvalue = int(int_str, 0)
        if neg:
            pyvalue = -pyvalue
        self._add_constants(name, pyvalue)
        self._declare(f'macro {name}', pyvalue)
    ...
    def _process_macros(self, macros):
        for key, value in macros.items():
            ...
            if _r_int_literal.match(value):
                ...
                self._declare(f'macro {key}', value)
            else:
                raise CDefError(
        ...
        if isinstance(node, pycparser.c_ast.FuncDecl):
            ...
            if decl.name:
                ...
                if tp.is_raw_function:
                    ...
                    self._add_integer_constant(decl.name, f'-{decl.init.expr.value}')
                    ...
                    if (quals & model.Q_CONST) and not tp.is_array_type:
                        self._declare(f'constant {decl.name}', tp, quals=quals)
                    else:
                        _warn_for_non_extern_non_static_global_variable(decl)
                        self._declare(f'variable {decl.name}', tp, quals=quals)
    ...
    def _get_type_and_quals(self, typenode, name=None, partial_length_ok=False,
        ...
        if (
            ...
            and f'typedef {typenode.type.names[0]}' in self._declarations
        ):
            tp, quals = self._declarations[f'typedef {typenode.type.names[0]}']
        ...
        if isinstance(typenode, pycparser.c_ast.ArrayDecl):
            ...
            if typedef_example is not None:
                ...
                typedef_example = f"*{typedef_example}"
            #
        ...
        if isinstance(typenode, pycparser.c_ast.TypeDecl):
            ...
            if isinstance(type, pycparser.c_ast.IdentifierType):
                ...
                if names != ['signed', 'char']:    # keep this unmodified
                    ...
                    while names:
                        name = names[0]
                        if name not in ('short', 'long', 'signed', 'unsigned'):
                            break
                    ...
                    for prefix in ('unsigned', 'short', 'long'):
                        newnames.extend(prefix for _ in range(prefixes.get(prefix, 0)))
                    if not names:
                        names = ['int']    # implicitly
                    if names == ['int'] and (
                        'short' in prefixes or 'long' in prefixes
                    ):
                        names = []
    ...
    def _parse_function_type(self, typenode, funcname=None):
        ...
        abi = None
        if hasattr(typenode.type, 'quals') and typenode.type.quals[-3:] == [
            'volatile',
            'volatile',
            'const',
        ]:
