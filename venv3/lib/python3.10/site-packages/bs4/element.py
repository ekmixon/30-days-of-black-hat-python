...
class PageElement(object):
    ...
    def _find_one(self, method, name, attrs, string, **kwargs):
        return l[0] if (l := method(name, attrs, string, 1, **kwargs)) else None
...
class NavigableString(str, PageElement):
    ...
    def _all_strings(self, strip=False, types=PageElement.default):
        ...
        if types is not None and (
            ...
            and my_type is not types
            or not isinstance(types, type)
            and my_type not in types
        ):
            return
        value = self
        if strip:
...
class Tag(PageElement):
    ...
    def _all_strings(self, strip=False, types=PageElement.default):
        ...
        for descendant in self.descendants:
            ...
            if (
                isinstance(types, type)
                and descendant_type is not types
                or not isinstance(types, type)
                and types is not None
                and descendant_type not in types
            ):
    ...
    def find(self, name=None, attrs={}, recursive=True, string=None,
        ...
        return (
            l[0]
            if (l := self.find_all(name, attrs, recursive, string, 1, **kwargs))
            else None
        )
    findChild = find #BS2
